...
- `recognizedFocus.businessChainContext`：可选的业务链诊断。当前 `v1` 会列出链路节点角色、角色证据来源、最终动作、沿/功能块/输出能力以及通过共享变量关联的同 POU 能力；这些信息用于过滤重复沿、可靠同业务身份下的重复功能块、会绕过许可/状态/联锁条件的并联候选，并在证据可靠时增强业务 title/text 和排序。它不改变节点、端口或拓扑端点；分析失败或证据不足时继续使用原有建议逻辑。
