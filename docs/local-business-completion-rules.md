...
1. 建议必须保持合法拓扑和端口方向。
2. 建议的 `startNodes` 与 `endNodes` 不得重叠，也不得使现有图形成回路；无效边界必须在返回前过滤。
3. 具体函数或功能块必须存在于当前库数据。
4. 要求字符串、数值、TIME 或 `AXIS_REF` 的规则必须有相应局部数据类型证据。
5. 不得跨设备自动绑定变量；当前没有可靠设备 ID 时变量保持 `???`。
6. 不得自动生成急停、安全门、光栅、STO 或其他安全信号旁路。
